...
# import warnings
# import numpy as np
# # 代码原地址：https://github.com/Linwei-Chen/FreqFusion# 论文：Frequency-aware Feature Fusion for Dense Image Prediction# 论文地址：https://arxiv.org/pdf/2408.12879
...
#                     mask_lr_lr_feat = F.interpolate(carafe(mask_lr_lr_feat_lr, mask_lr_init, self.lowpass_kernel, self.up_group, 2), size=compressed_hr_feat.shape[-2:], mode='nearest')                    
...
#                     mask_hr_lr_feat = F.interpolate(carafe(self.content_encoder2(compressed_lr_feat), mask_lr_init, self.lowpass_kernel, self.up_group, 2), size=compressed_hr_feat.shape[-2:], mode='nearest')                    
...
#                 hr_feat = hr_feat_hf
#         if self.feature_resample:            
#             # print(lr_feat.shape)            l
#             r_feat = self.dysampler(hr_x=compressed_hr_feat,lr_x=compressed_lr_feat, feat2sample=lr_feat)          
...
#             elif self.direction_feat == 'sim_concat':                self.direct_scale = nn.Conv2d(in_channels + local_window**2 - 1, out_channels, kernel_size=kernel_size, padding=kernel_size//2)            
#             else: raise NotImplementedError            
...
#         coords = F.pixel_shuffle(coords.view(B, -1, H, W), scale).view(B, 2, -1, scale * H, scale * W).permute(0, 2, 3, 4, 1).contiguous().flatten(0, 1)        
#         return F.grid_sample(x.reshape(B * self.groups, -1, x.size(-2), x.size(-1)), coords, mode='bilinear',align_corners=False, padding_mode="border").view(B, -1, scale * H, scale * W)        
...
#     参数：    - input_tensor: 输入张量，形状为[B, C, H, W]    - k: 范围大小，表示周围KxK范围内的点
#     返回：    - 输出张量，形状为[B, KxK-1, H, W]    """    
...
#     print(f"Using device: {device}")    # 初始化模型并移动到设备    
#     ff = FreqFusion(hr_channels=64, lr_channels=64).to(device)    # 创建随机输入并移动到设备    
#     hr_feat = torch.rand(1, 64, 32, 32).to(device)    
#     lr_feat = torch.rand(1, 64, 16, 16).to(device)    # 前向传播    
#     _, hr_feat, lr_feat = ff(hr_feat=hr_feat, lr_feat=lr_feat) 
#     # lr_feat [1, 64, 32, 32]    
#     # 打印输出的形状    
...
#     # _, hr_feat, lr_feat = m(hr_feat=hr_feat, lr_feat=lr_feat).cuda()
#     # 打印输出的形状    
#     print(hr_feat.shape)    
#     print(lr_feat.shape)
