...
class PaCoLoss(nn.Module):
    ...
    def forward(self, features, labels=None, sup_logits=None):
        ...
        # add ground truth
        one_hot_label = torch.nn.functional.one_hot(labels[:batch_size, ].view(-1, ), n_classes=self.n_classes).to(
            torch.float32)
        mask = torch.cat((one_hot_label * self.beta, mask * self.alpha), dim=1)

        # compute log_prob
        logits_mask = torch.cat((torch.ones(batch_size, self.n_classes).to(device), self.gamma * logits_mask), dim=1)
